...
const Bloc6 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Récupère le token d'authentification depuis le localStorage (si l'utilisateur est authentifié).
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/send-email`, {
        ...
        headers: {
          ...
          ...(token && { 'Authorization': `Bearer ${token}` })  // Si un token est présent, il est ajouté aux headers pour l'authentification.
        },
        body: JSON.stringify({
          from_name: name,
          from_email: email,
      ...
      // Si la requête est réussie, affiche un message de succès et réinitialise les champs du formulaire.
      if (response.ok) {
      ...
      <div className="contact-form">
        <form onSubmit={handleSubmit}>
          ...
          <input
            ...
            placeholder="Name"
            value={name}
            onChange={(e) => setName(e.target.value)} // Met à jour l'état du nom à chaque modification.
            required
            aria-label="Name"
            aria-required="true"
          />
        ...
        {/* Affichage du message de succès ou d'erreur après l'envoi du formulaire */}
